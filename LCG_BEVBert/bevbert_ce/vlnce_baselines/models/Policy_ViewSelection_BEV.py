...
class BEV(Net):
    def __init__(
        ...
        self.use_learnable_fusion_lambda = bool(
            getattr(pc_cfg, "enable_learnable_fusion_lambda", False)
        ) if pc_cfg is not None else False
        self.pc_accumulate_mode = str(getattr(pc_cfg, "accumulate_mode", "mean")).lower() if pc_cfg is not None else "mean"
        self.pc_fusion_scope = str(getattr(pc_cfg, "fusion_scope", "connected")).lower() if pc_cfg is not None else "connected"
            ...
            if (
                ...
                if gmap_pc_points.ndim == 4:
                    ...
                    if pc_points.numel() > 0:
                        ...
                        pc_feat = self.pointnet_encoder(pc_points)
                        ...
                        gmap_img_fts_nav = gmap_img_fts + delta
                else:
                    ...
                    pc_nonzero = torch.nonzero(effective_pc_masks, as_tuple=False)
                    if pc_nonzero.numel() > 0:
                        ...
                        if fusion_lambda is not None:
                            pc_feat = pc_feat * fusion_lambda
                        token_linear = pc_nonzero[:, 0] * glen + pc_nonzero[:, 1]
