...
class RLTrainer(BaseVLNCETrainer):
    ...
    def _get_fusion_lambda_for_log(self):
        """Fusion strength for logging: sigmoid(lambda_raw), 1.0 if fused but not learnable, or None."""
        try:
            net = self._get_policy_net_module()
        except Exception:
            return None
        pc = getattr(self.config.MODEL, "POINT_CLOUD", None)
        if pc is None or not bool(getattr(pc, "enable_fusion", False)):
            return None
        raw = getattr(net, "fusion_lambda_raw", None)
        if raw is not None:
            return float(torch.sigmoid(raw.detach()).item())
        if not bool(getattr(pc, "enable_learnable_fusion_lambda", True)):
            return 1.0
        return None

    def _log_il_sft_config(self):
        if self.local_rank >= 1:
            return
        il = self.config.IL
        logger.info("========== IL / SFT config (BEV) ==========")
        logger.info(
            "iters=%s log_every=%s lr=%s batch_size=%s ml_weight=%s expert_policy=%s",
            il.iters, il.log_every, il.lr, il.batch_size, il.ml_weight, il.expert_policy,
        )
        logger.info(
            "sample_ratio (initial)=%s decay_interval=%s warmup_iters=%s min_lr_ratio=%s",
            il.sample_ratio, il.decay_interval,
            getattr(il, "warmup_iters", 0), getattr(il, "min_lr_ratio", 1.0),
        )
        logger.info(
            "pointnet_lr=%s finetune_stage=%s is_requeue=%s load_from_ckpt=%s",
            getattr(il, "pointnet_lr", -1),
            getattr(il, "finetune_stage", "full"),
            bool(il.is_requeue),
            bool(il.load_from_ckpt),
        )
        if getattr(il, "load_from_ckpt", False):
            logger.info("ckpt_to_load=%s", getattr(il, "ckpt_to_load", ""))
        logger.info(
            "max_traj_len=%s max_text_len=%s loc_noise=%s waypoint_aug=%s ghost_aug=%s back_algo=%s",
            il.max_traj_len, il.max_text_len, il.loc_noise, il.waypoint_aug, il.ghost_aug, il.back_algo,
        )
        logger.info("============================================")

    def _log_eval_policy_context(self, checkpoint_path: str):
        if self.local_rank >= 1:
            return
        lam = self._get_fusion_lambda_for_log()
        pc = getattr(self.config.MODEL, "POINT_CLOUD", None)
        logger.info("========== Eval / infer policy context ==========")
        logger.info("checkpoint_path=%s", checkpoint_path)
        logger.info("finetune_stage=%s", getattr(self.config.IL, "finetune_stage", "full"))
        if pc is not None:
            logger.info(
                "POINT_CLOUD: depth_pc=%s pointnet=%s fusion=%s scope=%s learnable_lambda=%s",
                bool(pc.enable_depth_to_pointcloud),
                bool(pc.enable_pointnet),
                bool(pc.enable_fusion),
                str(getattr(pc, "fusion_scope", "connected")),
                bool(getattr(pc, "enable_learnable_fusion_lambda", False)),
            )
        logger.info(
            "fusion_lambda=%s",
            f"{lam:.6f}" if lam is not None else "n/a",
        )
        print(
            f"[eval/infer] checkpoint={checkpoint_path} fusion_lambda="
            f"{(lam if lam is not None else 'n/a')}"
        )
        logger.info("========================================")
    ...
    def save_checkpoint(self, iteration: int):
        ...
        ckpt_path = os.path.join(self.config.CHECKPOINT_FOLDER, f"ckpt.iter{iteration}.pth")
        torch.save(obj=payload, f=ckpt_path)
        if self.local_rank < 1:
            lam = self._get_fusion_lambda_for_log()
            lam_str = f"{lam:.6f}" if lam is not None else "n/a"
            logger.info("[checkpoint] saved iter=%s path=%s fusion_lambda=%s", iteration, ckpt_path, lam_str)
            print(f"[checkpoint] iter={iteration} fusion_lambda={lam_str} | saved -> {ckpt_path}")
    ...
    def _log_module_status_once(self):
        if self._module_status_logged_once or self.local_rank >= 1:
            return
        if not hasattr(self.config.MODEL, "POINT_CLOUD"):
        ...
        logger.info("========== PointCloud/PointNet (BEV) ==========")
        logger.info(
            "POINT_CLOUD: depth_pc=%s crop=%s max_depth_m=%s num_pts=%s pointnet=%s fusion=%s scope=%s",
            bool(pc_cfg.enable_depth_to_pointcloud),
            bool(pc_cfg.enable_spatial_crop),
            float(pc_cfg.max_depth_m),
            int(pc_cfg.num_points),
            bool(pc_cfg.enable_pointnet),
            bool(pc_cfg.enable_fusion),
            str(pc_cfg.fusion_scope),
        )
        logger.info(
            "pointnet_built=%s proj_built=%s pointnet_params=%s fusion_params=%s",
            getattr(net_module, "pointnet_encoder", None) is not None,
            getattr(net_module, "pc_feat_proj", None) is not None,
            pointnet_param_count,
            fusion_param_count,
        )
        logger.info("===============================================")
        self._module_status_logged_once = True
    ...
    def _initialize_policy(
        ...
        if bool(config.IL.is_requeue) and setup_optimizer:
            ...
            base_decay_params = [p for n, p in base_param_optimizer if not any(nd in n for nd in no_decay)]
            base_no_decay_params = [p for n, p in base_param_optimizer if any(nd in n for nd in no_decay)]
            if len(base_decay_params) > 0:
                optimizer_grouped_parameters.append({
                    'params': base_decay_params, 'weight_decay': 0.01, 'lr': float(self.config.IL.lr),
                })
            if len(base_no_decay_params) > 0:
                optimizer_grouped_parameters.append({
                    'params': base_no_decay_params, 'weight_decay': 0.0, 'lr': float(self.config.IL.lr),
                })
            pointnet_decay_params = [p for n, p in pointnet_param_optimizer if not any(nd in n for nd in no_decay)]
            pointnet_no_decay_params = [p for n, p in pointnet_param_optimizer if any(nd in n for nd in no_decay)]
            if len(pointnet_decay_params) > 0:
                optimizer_grouped_parameters.append({
                    'params': pointnet_decay_params, 'weight_decay': 0.01, 'lr': pointnet_lr,
                })
            if len(pointnet_no_decay_params) > 0:
                optimizer_grouped_parameters.append({
                    'params': pointnet_no_decay_params, 'weight_decay': 0.0, 'lr': pointnet_lr,
                })
            ...
            if self.local_rank < 1:
                logger.info(
                    f"SFT requeue: base_lr={float(self.config.IL.lr):.6g} pointnet_lr={pointnet_lr:.6g}"
                )
        ...
        if load_from_ckpt:
            ...
            if self.local_rank < 1:
                logger.info("Checkpoint load: missing=%d unexpected=%d",
                            len(incompatible_keys.missing_keys), len(incompatible_keys.unexpected_keys))
        ...
        if not bool(config.IL.is_requeue) and setup_optimizer:
            finetune_stage = str(getattr(self.config.IL, "finetune_stage", "full")).lower()
            if finetune_stage == "warmup":
                for n, p in self.policy.named_parameters():
                    p.requires_grad_(any(k in n for k in pointnet_name_keywords))
                if self.local_rank < 1:
                    logger.info("SFT finetune_stage=warmup: PointNet/fusion only.")
            elif finetune_stage == "align":
                # Same trainable set as default BEV training + PointCloud branch: all params on,
                # then re-apply fix_lang_embedding / fix_pano_embedding (same rules as vilmodel init).
                net = self._get_policy_net_module()
                mc = config.MODEL
                ...
                if self.local_rank < 1:
                    logger.info(
                        "SFT finetune_stage=align: same backward coverage as default train + PointCloud "
                        "(fix_lang_embedding=%s fix_pano_embedding=%s).",
                        bool(mc.fix_lang_embedding),
                        bool(mc.fix_pano_embedding),
                    )
            elif finetune_stage == "full":
                pass
            else:
                raise ValueError(
                    f"Unknown IL.finetune_stage: {finetune_stage!r} (expected full, warmup, align)."
                )
            ...
            if finetune_stage in ("warmup", "align"):
                ...
                if len(pointnet_decay_params) > 0:
                    optimizer_grouped_parameters.append({
                        'params': pointnet_decay_params, 'weight_decay': 0.01, 'lr': float(pointnet_lr),
                    })
                if len(pointnet_no_decay_params) > 0:
                    optimizer_grouped_parameters.append({
                        'params': pointnet_no_decay_params, 'weight_decay': 0.0, 'lr': float(pointnet_lr),
                    })
                if setup_optimizer:
                    if len(optimizer_grouped_parameters) == 0:
                        raise ValueError("No trainable parameters for optimizer (check finetune_stage).")
                    ...
                    if self.local_rank < 1:
                        logger.info(
                            f"SFT optimizer: base_lr={float(self.config.IL.lr):.6g} pointnet_lr={float(pointnet_lr):.6g} "
                            f"stage={finetune_stage}"
                        )
            else:
                if setup_optimizer:
    ...
    def _nav_gmap_variable(self, cur_vp, cur_pos, cur_ori):
        ...
        batch_no_vp_left = []
        batch_gmap_pc_points, batch_gmap_pc_masks, batch_gmap_pc_fusion_masks = [], [], []

        pc_cfg = getattr(self.config.MODEL, "POINT_CLOUD", None)
        use_pc = bool(pc_cfg is not None and pc_cfg.enable_depth_to_pointcloud)
        num_points = int(pc_cfg.num_points) if use_pc else 0
        fusion_scope = str(getattr(pc_cfg, "fusion_scope", "connected")).lower() if use_pc else "connected"
                ...
                for vp in ghost_vp_ids:
                    ...
                    if pc_list is not None and len(pc_list) > 0:
                        ...
                        token_pc_fusion_masks.append(
                            torch.full((max_pc_samples,), is_connected_ghost, dtype=torch.bool, device=self.device)
                        )
                    else:
                        token_pc_points.append(zero_pc)
                        token_pc_masks.append(torch.zeros(max_pc_samples, dtype=torch.bool, device=self.device))
                        token_pc_fusion_masks.append(torch.zeros(max_pc_samples, dtype=torch.bool, device=self.device))
                batch_gmap_pc_points.append(torch.stack(token_pc_points, dim=0))
                batch_gmap_pc_masks.append(torch.stack(token_pc_masks, dim=0))
                batch_gmap_pc_fusion_masks.append(torch.stack(token_pc_fusion_masks, dim=0))
        ...
        if use_pc:
            ...
            gmap_pc_masks = torch.zeros(bs, max_gmap_len, max_pc_samples, dtype=torch.bool, device=self.device)
            gmap_pc_fusion_masks = torch.zeros(bs, max_gmap_len, max_pc_samples, dtype=torch.bool, device=self.device)
            for i in range(bs):
    ...
    def train(self):
        ...
        self._log_il_sft_config()
        lam0 = self._get_fusion_lambda_for_log()
        if self.local_rank < 1 and lam0 is not None:
            logger.info("[train] initial fusion_lambda=%.6f", lam0)
            print(f"[train] initial fusion_lambda={lam0:.6f}")
        ...
        for idx in range(start_iter, total_iter, log_every):
            ...
            if self.local_rank < 1:
                ...
                for k, v in logs.items():
                    if k == "fusion_lambda" and len(v) > 0:
                        logs[k] = float(v[-1])
                    else:
                        logs[k] = np.mean(v)
                ...
                lam_cur = self._get_fusion_lambda_for_log()
                if lam_cur is not None:
                    logger.info(f"fusion_lambda: {lam_cur:.6f}")
                    print(f"[train] iter={cur_iter} lr={current_lr} fusion_lambda={lam_cur:.6f}")
                else:
                    print(f"[train] iter={cur_iter} lr={current_lr} fusion_lambda=n/a")
    ...
    def _eval_checkpoint(
        ...
        self._log_eval_policy_context(checkpoint_path)
        self.policy.eval()
        self.waypoint_predictor.eval()
    ...
    def rollout(self, mode, ml_weight=None, sample_ratio=None):
        ...
        for stepk in range(self.max_len):
            ...
            # BEV metric map pointcloud (existing)
            pc, pc_mask, pc_feat = self.lift(cur_pos, cur_ori, wp_outputs['pano_rgb_grid'], wp_outputs['pano_depth_grid'])

            pc_m = getattr(self.config.MODEL, "POINT_CLOUD", None)
            do_pc_sampling = bool(pc_m is not None and pc_m.enable_depth_to_pointcloud)
            if do_pc_sampling:
                ...
                num_points_pc = int(pc_m.num_points)
                max_depth_m = float(pc_m.max_depth_m)
                enable_spatial_crop = bool(pc_m.enable_spatial_crop)
                fps_seed_base = int(pc_m.fps_seed) if int(pc_m.fps_seed) >= 0 else int(self.config.TASK_CONFIG.SEED)
            else:
            ...
            if mode == 'train' and getattr(self.config.MODEL, "POINT_CLOUD", None) is not None:
                pc_cfg = self.config.MODEL.POINT_CLOUD
