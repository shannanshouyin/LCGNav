...
flag3=" --exp_name release_r2r_dagger
      --run-type eval
      --exp-config run_r2r/iter_train.yaml
      SIMULATOR_GPU_IDS [0]
      TORCH_GPU_IDS [0]
      ...
      NUM_ENVIRONMENTS 8
      TASK_CONFIG.SIMULATOR.HABITAT_SIM_V0.ALLOW_SLIDING True
      EVAL.CKPT_PATH_DIR data/logs/checkpoints/release_r2r_dagger/
...
case $mode in 
      ...
      infer)
      echo "###### infer mode ######"
      python -m torch.distributed.launch --nproc_per_node=1 --master_port $2 run.py $flag4
      ;;
esac

# 命令行运行：
# CUDA_VISIBLE_DEVICES=0 bash run_r2r/main_server.bash dagger 2333
# CUDA_VISIBLE_DEVICES=0 bash run_r2r/main_server.bash grpo 2333
# CUDA_VISIBLE_DEVICES=0 bash run_r2r/main_server.bash eval 2333
# CUDA_VISIBLE_DEVICES=0 bash run_r2r/main_server.bash infer 2333
