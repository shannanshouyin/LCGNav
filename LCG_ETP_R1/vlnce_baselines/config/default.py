...
# A dedicated LR for newly introduced PointNet/fusion layers in SFT.
# If <= 0, fallback to IL.lr.
_C.IL.pointnet_lr = -1.0
# Fine-tuning stage control for SFT post-training:
# - "warmup": freeze all loaded weights, train only newly introduced PointNet/fusion layers
# - "align": continue from warmup checkpoint and train the full model
...
# Align with run_rxr/rxr_vlnce.yaml (DATA_PATH / NDTW.GT_PATH use RxR_VLNCE_v0_enc_xlmr).
# Older releases used RxR_VLNCE_v0; override locally if your dataset layout differs.
...
_C.MODEL.POINT_CLOUD.only_candidate_views = True
_C.MODEL.POINT_CLOUD.accumulate_mode = "mean"
# Ghost fusion scope:
# - "global": fuse all ghost tokens in the global map.
# - "connected": fuse only one-hop ghosts connected to current node.
_C.MODEL.POINT_CLOUD.fusion_scope = "global"
# Learnable global scalar lambda for ghost-node fusion:
#   f = f_orig + lambda * W_g(g)
# If disabled, lambda is treated as 1.0 (no additional parameter).
_C.MODEL.POINT_CLOUD.enable_learnable_fusion_lambda = False
_C.MODEL.POINT_CLOUD.fusion_lambda_init = 0.1
# Base random seed for FPS sampling reproducibility.
# If < 0, fallback to TASK_CONFIG.SEED.
...
# Forward-time depth truncation before/for depth encoder in waypoint stage.
# mode:
# - "off": disable truncation
# - "global": truncate all 12-view depth maps to max_depth_m
# - "candidate": truncate only selected candidate-view depth maps to max_depth_m
_C.MODEL.DEPTH_TRUNCATION = CN()
_C.MODEL.DEPTH_TRUNCATION.mode = "off"
_C.MODEL.DEPTH_TRUNCATION.max_depth_m = 3.0
# Set these to match TASK_CONFIG.SIMULATOR.DEPTH_SENSOR when depth is normalized.
_C.MODEL.DEPTH_TRUNCATION.normalize_depth = True
_C.MODEL.DEPTH_TRUNCATION.min_depth_m = 0.0
_C.MODEL.DEPTH_TRUNCATION.max_sensor_depth_m = 10.0
...
_C.MODEL.SAP = CN()
# If True, mask visited (real) nodes in global SAP scoring, so planning targets
# are restricted to [STOP] + ghost/frontier nodes.
_C.MODEL.SAP.mask_visited = True
