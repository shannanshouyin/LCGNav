...
class BaseVLNCETrainer(BaseILTrainer):
            ...
            if os.path.isfile(self.config.EVAL.CKPT_PATH_DIR):
                ...
                while True:
                    ...
                    while current_ckpt is None:
                        current_ckpt = self._poll_pth_checkpoint_folder(
    ...
    def get_ckpt_id(self, ckpt_path):
        ...
        # Prefer the canonical format: ckpt.iter{N}.pth
        match = re.search(r"ckpt\.iter(\d+)\.pth$", ckpt_path)
        if match is not None:
            return int(match.group(1))

        # Fallback: extract the last integer in filename to avoid crashing on
        # non-standard but valid checkpoint names.
        nums = re.findall(r"\d+", ckpt_path)
        if len(nums) > 0:
            return int(nums[-1])

        logger.warning(f"Cannot parse checkpoint id from filename: {ckpt_path}, fallback to 0.")
        return 0

    @staticmethod
    def _poll_pth_checkpoint_folder(checkpoint_folder: str, previous_ckpt_ind: int):
        """Return the next checkpoint path considering only `.pth` files."""
        if not os.path.isdir(checkpoint_folder):
            return None
        ckpt_paths = glob.glob(os.path.join(checkpoint_folder, "*.pth"))
        ckpt_paths.sort(key=os.path.getmtime)
        ind = previous_ckpt_ind + 1
        if ind < len(ckpt_paths):
            return ckpt_paths[ind]
        return None
