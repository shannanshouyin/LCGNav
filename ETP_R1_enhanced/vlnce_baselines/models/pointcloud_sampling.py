...
def fps(points: torch.Tensor, n_samples: int, seed: int) -> torch.Tensor:
    """
    Farthest Point Sampling (FPS) with deterministic numpy RNG.
...
def build_depth_feats_maps(observations: dict, num_imgs: int = 12) -> torch.Tensor:
    """
    Build a (B, num_imgs, H, W, 1) depth tensor aligned with the image ordering used in
    `vlnce_baselines.models.R1Policy.ETP.forward(mode='waypoint')` for `depth_feats`.

    This reproduces the same depth batching + clockwise/counter-clockwise flip logic.
    """

    # Find a representative depth tensor to infer shape (ETP.forward uses observations['depth']).
    depth_ref = None
    ...
    depth_batch = torch.zeros_like(depth_ref).repeat(num_imgs, 1, 1, 1)  # (B*num_imgs,H,W,1)

    a_count = 0
    # IMPORTANT: keep the exact same ordering logic as ETP.forward(mode='waypoint').
    for k, v in observations.items():
    ...
    depth_maps = depth_batch.view(b, num_imgs, h, w, c)  # (B,num_imgs,H,W,1)

    # Mirror the flip applied to depth embeddings in ETP.forward.
