...
def fps(points: torch.Tensor, n_samples: int, seed: int) -> torch.Tensor:
    """
    Farthest Point Sampling (FPS) with deterministic torch RNG.
...
def build_depth_feats_maps(observations: dict, num_imgs: int = 12) -> torch.Tensor:
    """
    Build a (B, num_imgs, H, W, 1) depth tensor aligned with PolicyViewSelectionBEV
    `forward(mode='waypoint')` depth_feats ordering (counter-clockwise after flip).
    """
    depth_ref = None
