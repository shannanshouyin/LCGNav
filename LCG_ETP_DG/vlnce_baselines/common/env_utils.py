...
def construct_envs(
    ...
    # When EPISODES_ALLOWED is a small subset, get_scenes_to_load may return fewer unique
    # scenes than parallel envs; the old guard raised here. Duplicate the full scene list
    # per worker so each env is valid; EPISODES_ALLOWED still restricts which episodes run.
    few_scenes_multi_env = (
        num_envs > 1
        and len(scenes) > 0
        and len(scenes) < num_envs
        and len(scenes) != 1
    )
    ...
    if num_envs > 1:
        ...
        if few_scenes_multi_env:
            logger.warning(
                "Fewer CONTENT_SCENES (%d) than parallel envs (%d); assigning the full scene "
                "list to each worker (e.g. EPISODE_ID subset). Consider NUM_ENVIRONMENTS=1 to "
                "reduce memory.",
                len(scenes),
                num_envs,
            )

    if not few_scenes_multi_env and len(scenes) > 1 and num_envs > 1:
        random.shuffle(scenes)

    if len(scenes) == 1:
        scene_splits = [[scenes[0]] for _ in range(num_envs)]
    elif few_scenes_multi_env:
        scene_splits = [list(scenes) for _ in range(num_envs)]
    else:
