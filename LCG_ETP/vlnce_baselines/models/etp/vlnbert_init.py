...
def get_vlnbert_models(config=None):
    ...
    vis_config.use_dynamic_graph = getattr(config, 'use_dynamic_graph', False)
    
    # Geometric dropout configuration (only effective when use_dynamic_graph=True)
    vis_config.use_geo_dropout = getattr(config, 'use_geo_dropout', False)
    vis_config.geo_dropout_prob = getattr(config, 'geo_dropout_prob', 0.3)
    
    # Geometry-conditioned semantic edge configuration (only effective when use_dynamic_graph=True)
    vis_config.use_geo_conditioned_semantic = getattr(config, 'use_geo_conditioned_semantic', False)
    
    # Node gating configuration
    vis_config.use_node_gating = getattr(config, 'use_node_gating', False)
